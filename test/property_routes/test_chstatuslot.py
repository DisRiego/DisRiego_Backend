...
async def test_activate_lot_in_active_property():
    ...
    try:
        ...
        # Activar lote via API (se asume que la API está correctamente implementada para manejar la activación)
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            # Verificar que el estado del lote se haya actualizado correctamente a activo (state=18)
            db.refresh(lot)
            assert lot.state == 18
