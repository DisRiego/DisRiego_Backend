...
async def test_activate_already_active_lot():
    ...
    try:
        ...
        # Crear lote con estado 18 (activo) y asociarlo al predio
        lot = Lot(
        ...
        # Intentar activar el lote nuevamente (lo cual no debería ser permitido porque ya está activo)
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.put(f"/properties/lot/{lot.id}/state", data={
                "new_state": True  # Intentamos activar el lote que ya está activo
            })

            # Verificamos que la respuesta sea exitosa (estado 200) y que el lote no haya cambiado de estado
            assert response.status_code == 200
            assert response.json()["data"]["state"] == 18  # El lote debería seguir activo (18)

            # Verificar que el estado del lote siga siendo activo
            db.refresh(lot)
            assert lot.state == 18  # Verificamos que el lote siga activo
