...
@pytest.mark.asyncio
async def test_delete_type_crop(db: Session, type_crop_data: dict, payment_interval: PaymentInterval):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # Realizar la solicitud para eliminar el tipo de cultivo
        response = await client.delete(f"/my-company/type-crops/{type_crop_id}")
        
        # Validaciones para la eliminación
        assert response.status_code == 200, response.text
        assert response.json()["message"] == "Tipo de cultivo eliminado correctamente"
        
        # Verificar que el tipo de cultivo ya no exista en la base de datos
        deleted_type_crop = db.query(TypeCrop).filter(TypeCrop.id == type_crop_id).first()
        assert deleted_type_crop is None

    # 🧹 Limpieza: aunque ya se elimina en la prueba, asegurar que se elimine cualquier rastro en la base de datos
